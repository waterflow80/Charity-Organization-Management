...
public class MemberDAO {
	...
	public static int addMember(Member member) {
		...
		// Query
		String query = "INSERT INTO Member(firstName, lastName, nationalIdNumber, role, addressId, phoneNumber, profilePhotoPath) VALUES "
				+ "(?,?,?,?,?,?,?)";
		
		try {
			// create the prepared statement
			PreparedStatement ps = connection.prepareStatement(query, Statement.RETURN_GENERATED_KEYS);
			ps.setString(1, member.getFisrtName());
			...
			ps.setString(7, member.getProfilePhoto().getPhotoPath());
	...
	public static Member getMemberById(int memberId) {
		...
		try {
			...
			if (rs.next()) {
				member.setMemberId(memberId);
				member.setFisrtName(rs.getString("firstName"));
	...
	/**
	 * Return a list of all members stored in the database*/
